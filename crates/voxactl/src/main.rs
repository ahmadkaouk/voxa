...
fn run() -> Result<(), String> {
    ...
    match command.as_str() {
        ...
        _ => {
            return Err(format!(
                "Unknown command '{command}'. Run 'voxactl help' for usage."
            ));
        }
    }
...
fn print_usage() {
    println!("voxactl v{}", voxa_core::version());
    println!("Usage:");
    println!("  voxactl health");
    println!("  voxactl status");
    println!("  voxactl start [manual|hotkey_toggle|hotkey_hold]");
    println!("  voxactl stop [manual|hotkey_toggle|hotkey_hold_release|max_duration]");
    println!("  voxactl config get");
    println!("  voxactl config set <key> <value>");
    println!("  voxactl api-key status");
    println!("  voxactl api-key set <value>");
    println!("  voxactl events");
...
impl IpcClient {
    fn connect(socket_path: &PathBuf) -> io::Result<Self> {
        ...
        let hello = ClientEnvelope::Hello(HelloRequest {
            ...
            client: Some("voxactl".to_owned()),
            client_version: Some(voxa_core::version().to_owned()),
        });
